...
Java_com_louisgeek_ffmpeg_FFmpegUtil_convertTsToMp4(JNIEnv *env, jclass thiz,
    ...
    //打开输入文件
    AVFormatContext *inputCtx = nullptr; //输入文件格式上下文
    ...
    //创建输出上下文
    AVFormatContext *outputCtx = nullptr; //输出文件格式上下文
    avformat_alloc_output_context2(&outputCtx, nullptr, nullptr, output);
    if (!outputCtx) {
        LOGE("无法创建输出上下文");
    ...
    while (av_read_frame(inputCtx, &packet) >= 0) {
        ...
        //转换时间基（PTS/DTS）,TS 和 MP4 的时间基可能不同，确保 PTS/DTS 正确转换
